/* CharacterCreator.jsx — Stardoll/Bratz-inspired creative-archetype builder.
   Flow: vibe → signature move → fuel → result card with email capture. */
      ...
      {options.map(o => {
        ...
        return (
          <button key={o.id} onClick={() => onPick(o.id)} style={{
            ...
            {o.emoji && <span style={{ width: 34, height: 34, flex: "none", borderRadius: 999, background: o.c || "var(--amber-900)",
              border: "2px solid var(--ink)", display: "inline-flex", alignItems: "center", justifyContent: "center",
              color: "var(--ink)", fontSize: 16 }}>{o.emoji}</span>}
            {o.label}
          </button>
        );
      })}
      ...
      {labels.map((l, i) => (
        <div key={l} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 700,
            color: i <= step ? "var(--crimson-500)" : "var(--fg3)" }}>{String(i + 1).padStart(2, "0")} {l}</span>
...
function ResultCard({ arche, vibe, fuel, onRestart }) {
  ...
  return (
    <div>
      {/* The shareable card */}
      <div style={{ background: a.c, color: a.fg, borderRadius: 26, border: "3px solid var(--ink)",
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "clamp(40px,6vw,64px)",
          lineHeight: .95, margin: "10px 0 6px", maxWidth: "80%" }}>{a.name}</h2>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 24, opacity: .9 }}>
          <img src={`${ASSET}/logos/monogram-red.png`} style={{ height: 34, filter: a.fg === "#fff" || a.fg === "var(--cream)" ? "brightness(0) invert(1)" : "none" }} alt="" />
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com"
                style={{ flex: "1 1 200px", fontFamily: "var(--font-sans)", fontSize: 16, padding: "13px 16px",
                  border: "2px solid var(--ink)", borderRadius: 14, outline: "none" }} />
...
function CharacterCreator({ onExit }) {
  ...
  return (
    <section style={{ minHeight: "calc(100vh - 63px)", padding: "44px 24px 80px",
      ...
      <div style={{ maxWidth: 620, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "clamp(36px,6vw,58px)",
            lineHeight: .98, margin: "10px 0 0", color: "var(--ink)" }}>
            What kind of creative<br /><span style={{ color: "var(--crimson-500)" }}>are you?</span>
          </h1>
        </div>

        <div style={{ background: "rgba(255,255,255,.7)", backdropFilter: "blur(6px)", borderRadius: 24,
...
Object.assign(window, { CharacterCreator });
