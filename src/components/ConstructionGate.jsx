...
export function ConstructionGate({ children }) {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => { if (nameRef.current) nameRef.current.style.opacity = '0.6'; }, 900);
    const t2 = setTimeout(() => {
      path.style.transition = 'fill 300ms ease';
      path.style.fill = '#F7F3EF';
      if (nameRef.current) nameRef.current.style.opacity = '1';
    }, 1600);
    const t3 = setTimeout(() => setPhase('prompt'), 2400);
  ...
  if (unlocked || gone) return children;

  return (
    <>
      <div style={{
        position: 'fixed', inset: 0,
        background: '#3C0606', zIndex: 999999,
        display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column',
        opacity: exiting ? 0 : 1,
        visibility: exiting ? 'hidden' : 'visible',
        transition: 'opacity 500ms ease, visibility 500ms ease',
      }}>
        <svg viewBox="0 0 174.05 211.72" xmlns="http://www.w3.org/2000/svg"
          style={{ width: 110, height: 'auto' }}>
          <path ref={pathRef} d={PATH_DATA}
            fill="none" stroke="#F7F3EF" strokeWidth="1.5"
        ...
        <div ref={nameRef} style={{
          ...
          color: '#F7F3EF', opacity: 0, marginTop: 20,
        ...
        {phase === 'prompt' && (
          <div style={{
            ...
            animation: 'gate-fade-in 400ms ease forwards',
          }}>
            <div style={{
              fontFamily: "'Space Grotesk', system-ui, sans-serif",
              fontSize: 9, letterSpacing: '0.24em', textTransform: 'uppercase',
              color: 'rgba(247,243,239,.3)', marginBottom: 30,
            }}>
              Under Construction
            </div>
            ...
            <div style={{
              ...
              borderBottom: `1px solid ${error ? 'rgba(251,100,100,.5)' : 'rgba(247,243,239,.22)'}`,
              transition: 'border-color 200ms ease',
              animation: error ? 'gate-shake 380ms ease' : 'none',
            }}>
              <input
                type="password"
                value={input}
                ...
                style={{
                  ...
                  fontFamily: "'Space Grotesk', system-ui, sans-serif",
                  fontSize: 22, letterSpacing: '0.14em', textAlign: 'center',
                  color: '#F7F3EF', padding: '10px 0', width: 180,
                  caretColor: '#FBD66D',
                }}
              />
              <button onClick={submit} style={{
                background: 'transparent', border: 'none',
                color: 'rgba(247,243,239,.38)', cursor: 'pointer',
                padding: '10px 2px 10px 10px', fontSize: 18, lineHeight: 1,
                transition: 'color 150ms ease',
              }}
                onMouseEnter={e => e.currentTarget.style.color = '#FBD66D'}
                onMouseLeave={e => e.currentTarget.style.color = 'rgba(247,243,239,.38)'}
              >→</button>
            </div>
          </div>
        )}
