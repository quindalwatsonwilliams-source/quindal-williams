import { ASSET, Eyebrow, Tag } from './ui';
...
function WorkSpread({ p, i }) {
  ...
  const Panel = (
    <div style={{
      flex: '1 1 0', minWidth: 0, position: 'relative', borderRadius: 24, overflow: 'hidden',
      ...
      <span style={{
        fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 26, color: p.fg,
        background: p.tex ? 'rgba(26,26,46,.78)' : 'transparent',
        padding: p.tex ? '6px 14px' : 0, borderRadius: 999,
      }}>
        {p.title}
      </span>
    </div>
  ...
  const Text = (
    <div style={{ flex: '1 1 0', minWidth: 0, padding: '8px 6px' }}>
      ...
      <h3 style={{
        ...
        fontSize: 'clamp(30px,3.6vw,46px)', lineHeight: 1.0,
        letterSpacing: '-.015em', margin: '12px 0 4px',
      }}>{p.title}</h3>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 18, flexWrap: 'wrap' }}>
        {p.tags.map(t => (
          <Tag key={t} bg="transparent" outline style={{ fontSize: 12, padding: '6px 13px' }}>{t}</Tag>
        ))}
      </div>
    </div>
  );
  return (
    <div className="work-spread" style={{
    ...
    <section id="work" style={{ padding: '80px 30px', scrollMarginTop: 70 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 'clamp(34px,5vw,60px)', margin: 0 }}>
            Selected work
          </h2>
